...
export WSE_GOL_MAX_FOSSIL_SETS_SPREAD=16
echo "WSE_GOL_MAX_FOSSIL_SETS_SPREAD=${WSE_GOL_MAX_FOSSIL_SETS_SPREAD}"
...
with SdkLauncher("./run", disable_version_check=True) as launcher:
    ...
    command = f"{env_prefix} cs_python client.py --ncycle 10000 --initial-state gosper --cmaddr %CMADDR% 2>&1 | tee run.log"
    ...
    response = launcher.run(
        ...
        r'\( -name "*.log" -o -name "*.pqt" -o -name "*.json" -o -name "*.npy" -o -name "*.cells"  -o -name "*.txt" \)',
    )
    logging.info("... done!")
    logging.info(response + "\n")
